...
Array.prototype.unique = function() {
  ...
  google.charts.setOnLoadCallback(drawBranchResvChart);
  google.charts.setOnLoadCallback(drawBranchSalesChart);
  // Callback that creates and populates a data table,
  // instantiates the pie chart, passes in the data and
  // draws it.
function drawBranchResvChart() {
	var resvData;
	var year = $("#year").val();
	var month = $("#month").val();
	...
	url = "/admin/stat/branchResv?year=" + year;
	if (month != '') {
    ...
    for (var i = 0; i < branchCount; i++) {
    	data.addColumn('number', branch[i]);
    }
    ...
    var chart = new google.visualization.BarChart(document.getElementById('branchResvChart'));
    chart.draw(data, options); 

  }
  
function drawBranchSalesChart() {
	var resvData;
	var year = $("#year").val();
	var month = $("#month").val();
	...
	url = "/admin/stat/branchPay?year=" + year;
	if (month != '') {
		url += "&month=" + leadingZeros(month, 2);
	}
    ...
    var chart = new google.visualization.ColumnChart(document.getElementById('branchSalesChart'));
    chart.draw(data, options); 
}
	...
	<div id="admin_userStat_wrapper">
		...
		<div id="page-wrapper">
			<section id="admin_userStat_section">
				<div class="row">
					<div class="col-lg-12">
						<h3 class="page-header">지점별 통계 페이지</h3>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
